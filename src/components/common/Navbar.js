...
const Navbar = () => {
    ...
    const fetchSublinks=async ()=>{
            try{
                ...
                const data=[];
                result.data.allTags.map((element)=>(
        ...
        NavbarLinks.map((element,index)=>(
                    ...
                    element.title === "Catalog" ? (
                        <div className='flex flex-row gap-1 items-center group relative'>
                            ...
                            <TiArrowSortedDown/>
                            <div className='invisible bg-richblack-5 p-4 absolute z-50 gap-2 flex flex-col rounded-lg
                            ...
                            <div className='bg-richblack-5 w-[40px] h-[40px] absolute rotate-45 -z-20 -translate-y-[50%] translate-x-[226%]'></div>
                            {
                                ...
                                subLinks.length?(
                                    subLinks.map((element,index)=>(
                                        <Link to={`${subLinks.link}`} key={index} className='hover:bg-richblack-50'>
                                            <p>{element}</p>
                        ...
                        <Link to={element.path} key={index}>
                            <li className={`${matchRoute(`${element.path}`)?"text-yellow-25":"text-white"}`}>{element.title}</li>
                        </Link>
                    )   
      ...
      <div className='flex flex-row gap-4 text-white items-center'>
        ...
        }
        {
            token===null &&(
                ...
                <Link to="/signup">
                    <p className='rounded-lg bg-richblack-800 p-2 text-richblack-200'>Sign up</p>
                </Link>
            )
        }
        {
